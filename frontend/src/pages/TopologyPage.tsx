  ...
  return (
    <>
      <PageTitle
        ...
        description="Controller, compute nodes, provider network, instances and the Cinder backend in one operational view."
        meta={[
          { label: 'Nodes', value: '3' },
      ...
      <div className="fade-up delay-1">
        <Panel num="07.A" title="Lab architecture" eyebrow="Controller / compute / storage / network" variant="flag">
          ...
          <div className="my-5 hatch-divider" />
...
function Connector({ label }: { label: string }) {
  return (
    <div className="relative flex flex-col items-center justify-center xl:px-2">
      ...
      <div className="relative bg-[#F7F2E2] border border-[#11100D]/30 px-3 py-2 text-center font-mono text-xs uppercase tracking-[0.08em] text-[#11100D]">
        {label}
      </div>
    </div>
  );
...
function TopologyNode({
  ...
  return (
    <div className={`relative bg-[#F7F2E2] border ${t.border} p-5`}>
      ...
      <Icon className="mb-5 h-6 w-6 text-[#11100D]" />
      <div className="font-display text-2xl font-semibold tracking-tight">{title}</div>
      <div className="label-compact mt-1">{subtitle}</div>
...
function TopologyTile({ icon: Icon, title, subtitle, detail, hint, code }: {
  ...
  return (
    <div className="bg-[#F7F2E2] p-4 relative">
      ...
      <div className="label-compact mt-0.5">{subtitle}</div>
      <div className="mt-3 text-sm text-[#11100D]"><Mono>{detail}</Mono></div>
      <div className="mt-1 text-xs text-[#6F6A5F]">{hint}</div>
    </div>
  );
}
...
function MiniList({ rows, primary, secondary }: { rows: AnyRecord[]; primary: string; secondary: string }) {
  if (!rows.length) {
    return <p className="text-sm text-[#6F6A5F]">No live records observed.</p>;
  }
  return (
    <div className="grid gap-1.5">
      {rows.slice(0, 8).map((row, index) => (
        <div
          key={index}
          className="grid grid-cols-[1fr_auto] items-center gap-3 border-b border-[#11100D]/10 py-2 last:border-0"
        >
